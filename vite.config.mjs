...
// address stays free and predictable. Override with TASKFORGE_PORT. The optional
// /etc/hosts alias (TASKFORGE_HOST, default `taskforge`) lets you open http://taskforge:7777
...
export default defineConfig({
  ...
  server: {
    ...
    allowedHosts: true,      // accept ANY hostname — safe because we bind to 127.0.0.1 only (not network-reachable),
                             // so you can point any /etc/hosts alias (taskforge, taskforge.test, …) at it with no config change
  },
});
